...
/**
 * PostRedirectClient — client half of /post/[postId]
 *
 * iOS intercepts navigation to theruncheck.app/post/{id} and opens the
 * RunCheck app directly (if installed) via Universal Links / AASA — see
 * public/.well-known/apple-app-site-association. This client component is
 * the fallback shown in a regular browser when:
 *   - The app is not installed, OR
 *   - Universal Links fail to activate (e.g. first launch before AASA caches)
 *
 * On load it immediately tries the custom URL scheme (runcheck://post/{id}).
 * If the app opens, the user leaves this page. If not, after 2 seconds it
 * redirects to the App Store. Mirrors app/open/page.tsx's proven pattern.
 */
...
export default function PostRedirectClient({ postId }: { postId: string }) {
  useEffect(() => {
    ...
    // belt-and-suspenders fallback, same as /open.
    window.location.href = `runcheck://post/${postId}`;
